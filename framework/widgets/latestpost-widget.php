...
class Latest_Posts_Widget extends WP_Widget {
	...
	function Latest_Posts_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_latest_posts', 'description' => esc_html__('The most recent posts with teaser text', 'betterwork') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 150, 'height' => 350, 'id_base' => 'latest-posts-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'latest-posts-widget', esc_html__('Betterwork: Latest Posts', 'betterwork'), $widget_ops, $control_ops );
	}
...
<?php			    while( $num_posts_query->have_posts()) : $num_posts_query->the_post();
				...
				<li>
<?php                               if ( $show_thumbs ) { include (O2_FUN . '/post-thumbnail.php'); } ?>
...
<?php				    if ( $show_more_link )  : ?>
                                        <a title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="read-more-align-right"><span><?php esc_html_e('Read more', 'betterwork'); ?></span> &rarr;</a>
<?php				    endif; ?>
				    <div class="clear"></div>
				</li>
<?php			    endwhile; ?>
...
<?php		endif;
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__('Latest Posts', 'betterwork'), 'category_id' => '', 'num_posts' => 3, 'post_offset' => 0, 'num_words_limit' => 0,'show_date_author' => false,'show_more_link' => false, 'show_thumbs' => false, 'thumb_frame_shadow' => false, 'post_thumb_width' => 60, 'post_thumb_height' => 60 );
		...
		<!-- Show Categories -->
		<p>
			<label for="<?php echo $this->get_field_id( 'category_id' ); ?>"><?php esc_html_e('Pick a specific category:', 'betterwork'); ?></label>
		...
		<!-- Number of Posts -->
		<p>
			<label for="<?php echo $this->get_field_id( 'num_posts' ); ?>"><?php esc_html_e('Number of posts to show:', 'betterwork'); ?></label>
			...
			<br />
			<small><?php esc_html_e('(at most 15)', 'betterwork'); ?></small>
		</p>

		<!-- Post Offset -->
		<!--<p>
			<label for="<?php echo $this->get_field_id( 'post_offset' ); ?>"><?php esc_html_e('Number of posts to skip:', 'betterwork'); ?></label>
			...
			<br />
			<small><?php esc_html_e('(offset from latest)', 'betterwork'); ?></small>
		</p>-->

		<!-- Number of Words Limit -->
		<p>
			<label for="<?php echo $this->get_field_id( 'num_words_limit' ); ?>"><?php esc_html_e('Limit the number of words to show from each post:', 'betterwork'); ?></label>
			...
			<br />
			<small><?php esc_html_e('(Could also be limited by "Excerpt Length" defined in the theme\'s options page)', 'betterwork'); ?></small>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_date_author' ); ?>">
			    ...
			    <?php esc_html_e('Show date & author info', 'betterwork'); ?>
			</label>
		</p>
		...
		<!--<p>
			<label for="<?php echo $this->get_field_id( 'show_more_link' ); ?>">
			    ...
			    <?php esc_html_e('Show "Read more &rarr;" link ', 'betterwork'); ?>
			</label>
		</p>-->
		...
		<!--<p>
			<label for="<?php echo $this->get_field_id( 'show_thumbs' ); ?>">
			    ...
			    <?php esc_html_e('Show thumbnails', 'betterwork'); ?>
			</label>
		</p>-->
                    ...
                    <div style="border: 1px solid #DDD; background-color: #F8F8F1; padding:7px;">
                        ...
                        <p>
                                <label for="<?php echo $this->get_field_id( 'thumb_frame_shadow' ); ?>">
                                    ...
                                    <?php esc_html_e('Show thumbnail frame shadow', 'betterwork'); ?>
                                </label>
                        </p>

                        <!-- Thumb Dimension -->
                        <p>
                                <label for="<?php echo $this->get_field_id( 'post_thumb_width' ); ?>"><?php esc_html_e('Thumbnail Dimensions:', 'betterwork'); ?></label><br />
                                ...
                                <br />
                                <?php esc_html_e('(Width X Height) in pixels', 'betterwork'); ?>
                        </p>
                    </div>
<?php           endif; ?>
