 ...
 * Description: A widget that allows to view image with text to be added to a sidebar.
 * Version: 1.0
 *
 */
 ...
 * @since 0.1
 */
add_action('widgets_init', 'o2_text_image_widgets');

/**
 * Register our widget.
 * 'o2_text_image_widgets' is the widget class used below.
 *
 * @since 0.1
 */
function o2_text_image_widgets() {
	register_widget('text_image');
	}

/**
 * Text image Widget class.
...
class text_image extends WP_Widget
{
    function text_image() 
    {
        $widget_ops = array( 
            ...
            'description' => __( 'Arbitrary text or HTML, with a simple image above text.', 'betterwork' )
        );

        $control_ops = array( 'id_base' => 'text-image', 'width' => 430 );

        $this->WP_Widget( 'text-image', __( 'Betterwork: Text With Image', 'betterwork' ), $widget_ops, $control_ops );      
    ...
    function form( $instance )
    {
        ...
        <p>
            <label>
                <strong><?php _e( 'Title', 'betterwork' ) ?>:</strong><br />
                <input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" />
            </label>
        </p>                  
        
        <p>
            <label><?php _e( 'Image', 'betterwork' ) ?>:
        ...
        <!--<p>
            <label><?php _e( 'Image Alignment', 'betterwork' ) ?>:
        ...
        <p>
            <label>
                ...
                <?php _e( 'Automatically add paragraphs', 'betterwork' ) ?>
            </label>
        </p>         
        <?php
    }
    ...
    function widget( $args, $instance )
    {
        ...
        $text = '<div class="text-image"><img src="' . betterwork_process_image( $instance['image'], 280, 152, 1, 100 ) . '" alt="' . $instance['title'] . '" /></div>' . $instance['text'];
