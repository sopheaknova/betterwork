...
$default_thumb = O2_IMG . '/common-images/default-thumb.png';
...
if ( function_exists('get_the_image') ) { 
	if ( of_get_option('show_thumbnail_in_archive') ) { // Default thumbnail option is selected
	...
	echo ( $the_thumb_html_as_array[src] ) ? '<span class="small-custom-frame alignleft'.$shadow_class.'"><a href="'.get_permalink().'" title="'.the_title('', '', false).'"><img src="'.betterwork_process_image( $the_thumb_html_as_array[url], $post_thumb_width, $post_thumb_height, 1, 100 ).'" width="'.$post_thumb_width.'" height="'.$post_thumb_height.'" alt="'.$the_thumb_html_as_array[alt].'" /></a></span>' : '';
	...
	if ( of_get_option('show_thumbnail_in_archive') ) { // Default thumbnail option is selected
	    if ( $the_post_thumbnail_link ) { // look for the thumbnail passed as a 'post_thumbnail' or 'thumbnail' custom field ?>
                    <span class="small-custom-frame alignleft<?php echo $shadow_class; ?>"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><img src="<?php echo betterwork_process_image( $the_post_thumbnail_link, $post_thumb_width, $post_thumb_height, 1, 100 ); ?>" width="<?php echo $post_thumb_width; ?>" height="<?php echo $post_thumb_height; ?>" alt="<?php the_title(); ?>" /></a></span>
...
<?php	    }
		...
		if ( $the_post_thumbnail_link ) { // look for the thumbnail passed as a 'post_thumbnail' or 'thumbnail' custom field 
			echo $the_post_thumbnail_link . ' Post thumbnail link' ;
		?>
		    <span class="small-custom-frame alignleft<?php echo $shadow_class; ?>"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><img src="<?php echo betterwork_process_image( $the_post_thumbnail_link, $post_thumb_width, $post_thumb_height, 1, 100 ); ?>" width="<?php echo $post_thumb_width; ?>" height="<?php echo $post_thumb_height; ?>" alt="<?php the_title(); ?>" /></a></span>
<?php	    }
	}
